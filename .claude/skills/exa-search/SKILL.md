...
## Project authentication (mandatory)

In this project, do not decide that Exa is unavailable merely because `EXA_API_KEY` is absent from the process environment. Before every Exa request, resolve the key from the environment first and then from the configured key file:

```bash
exa_api_key="${EXA_API_KEY:-$(cat /home/liqinsi/.config/exa/key 2>/dev/null)}"
if [ -z "$exa_api_key" ]; then
  printf 'Exa API key is not configured\n' >&2
  exit 1
fi
```

Use the key-resolution command exactly as written; do not replace `cat ... 2>/dev/null` with the Bash `$(< file 2>/dev/null)` shortcut, because that altered form yields an empty value. Use `-H "x-api-key: $exa_api_key"` for the request and run `unset exa_api_key` afterward. Never print the variable or the key file. A presence check of `EXA_API_KEY` alone is insufficient; always check `/home/liqinsi/.config/exa/key` before reporting that Exa is unavailable.
